...
import { X, ChevronLeft, ChevronRight, ArrowRightLeft, Circle, CheckCircle2, Plus, Trash2 } from "lucide-react";
import { STAGES, STAGE_META, CAT_META, SOURCE_META } from "../lib/meta";
import OwnerPicker from "./OwnerPicker";

export default function Drawer({ task, domain, people, onClose, onStage, onOwner, onHandoff, onToggleCheck, onAddCheck, onEditCheck, onRemoveCheck, onPatch, onDelete }) {
  const [title, setTitle] = useState(task.title);
  const [reason, setReason] = useState(task.reason || "");
  const [date, setDate] = useState((task.scheduled_at || task.due_at || "").slice(0, 10));
  const [newItem, setNewItem] = useState("");
  useEffect(() => {
    setTitle(task.title); setReason(task.reason || ""); setDate((task.scheduled_at || task.due_at || "").slice(0, 10));
  }, [task.id]);

  const stageIdx = STAGES.indexOf(task.stage);
  const commitDate = (v) => { setDate(v); onPatch(task.id, { scheduled_at: v ? `${v}T09:00:00` : null, due_at: null }); };
      ...
      <aside className="draw">
        ...
        <div className="db">
          ...
          <div className="navrow">
            <button className="btn" disabled={stageIdx === 0} onClick={() => onStage(task.id, STAGES[stageIdx - 1])}><ChevronLeft size={14} />Vorige</button>
