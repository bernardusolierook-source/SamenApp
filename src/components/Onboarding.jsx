...
export function Onboarding({ onDone }) {
  ...
  const go = async () => {
    ...
    try {
      ...
      onDone();
    } catch (e) { setErr(e.message || "Er ging iets mis."); } finally { setBusy(false); }
  };
    ...
    <div className="gate">
      <div className="panel">
        ...
        <div className="swatches">
          {PALETTE.map((c) => <div key={c} className="swatch" data-on={c === color ? 1 : 0} style={{ background: c }} onClick={() => setColor(c)} />)}
        </div>
        {mode === "create" ? (
          <><label>Naam van het huishouden</label><input type="text" value={hhName} onChange={(e) => setHhName(e.target.value)} /></>
        ) : (
          <><label>Uitnodigingscode</label><input type="text" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="ABC123" /></>
        )}
        {err && <div className="err">{err}</div>}
        <button className="bigbtn" onClick={go} disabled={busy}>{busy ? "Bezig…" : mode === "create" ? "Huishouden aanmaken" : "Aansluiten"}</button>
      </div>
    </div>
  );
}
