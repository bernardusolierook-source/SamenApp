...
export const fmtDate = (iso) =>
  iso
    ? new Intl.DateTimeFormat("nl-NL", { weekday: "short", day: "numeric", month: "short" }).format(new Date(iso))
    : null;

// ─────────────────────────────────────────────────────────────────────────
// Eigen, complete starterset voor een NL-gezin (~64 kaarten over de vijf
// categorieën van Fair Play). Bewust compleet: je herkent en streept weg,
// in plaats van zelf te moeten bedenken. Wat niet speelt -> archiveren (één tik).
// Niet overgenomen uit het boek; dit dekt de feitelijke levensgebieden zelf.
// ─────────────────────────────────────────────────────────────────────────
...
// Eenvoudige triage-suggestie: koppel een losse capture aan de meest waarschijnlijke
// kaart op basis van woordoverlap. Geen AI nodig voor v1 — dit verandert triage in één tik.
